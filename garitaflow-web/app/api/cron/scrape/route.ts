...
import { classifyRelative } from "@/lib/classifier"
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (const rd of readings) {
      ...
      const status = classifyRelative(estimatedWait, ha, hist?.sample_count ?? 0)
      await client.query(`INSERT INTO estimates(lane_type_id,estimated_wait,confidence,status)VALUES($1,$2,$3,$4)`,[lid,estimatedWait,confidence,status])
