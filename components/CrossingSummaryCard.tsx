...
function bestInGroup(lanes: LaneWaitTime[], categories: string[], excludeSentri = false) {
  return lanes
    .filter(l => categories.includes(l.category) && l.waitMinutes !== null && (!excludeSentri || l.icon !== 'sentri'))
    .sort((a, b) => (a.waitMinutes ?? 999) - (b.waitMinutes ?? 999))[0] ?? null
}

interface Props { crossing: BorderCrossing; pedwestClosed?: boolean }

export default function CrossingSummaryCard({ crossing, pedwestClosed }: Props) {
  const bestVehicle = bestInGroup(crossing.lanes, ['vehicle'], true)
  const pedCategories = pedwestClosed ? ['pedestrian'] : ['pedestrian', 'pedwest']
  const bestPedestrian = crossing.lanes
    .filter(l => pedCategories.includes(l.category) && l.name === 'General' && l.waitMinutes !== null)
    .sort((a, b) => (a.waitMinutes ?? 999) - (b.waitMinutes ?? 999))[0] ?? null
  const pedLabel = bestPedestrian?.category === 'pedwest' ? 'PedWest peatonal' : 'General / Ready Lane peatonal'
  ...
  return (
    <div className="bg-white rounded-xl shadow-card border border-surface-border p-4">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 flex flex-col gap-2">
        ...
        {bestPedestrian && (
          <p className="text-[13px] font-semibold text-brand-navy leading-snug">
            🚶 {pedLabel} — {bestPedestrian.waitMinutes} min
          </p>
        )}
        {showWarning && worst && (
          <p className="text-[11px] text-brand-blue mt-1">Evita {worst.name}: {worst.waitMinutes} min de espera</p>
        )}
      </div>
    </div>
  )
